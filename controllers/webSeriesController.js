const { getAllWebseries, getMovieById, createWebseries, deleteMovieById, updateWebseries } = require('../models/webSeries.js');
...
const getMovies = async (req, res) => {
    try {
        const movies = await getAllWebseries();
...
const addMovie = async (req, res) => {
    const videoMaster = await uploadFile(req?.files?.VideosUrl[0].path)
